...
description: "Look up driver settings, sandbox configuration, and files saved during a run."
section: reference
kind: reference
order: 20
---

Precedence for driver settings: **CLI flag → committed YAML → built-in default**. Per-run launch choices (`--once`, `--guarded`, `--host`, `--headless`, `--max-cost`, `--max-tokens`, `--theme`) are never written to YAML.

Overview: [Start here](/docs/start-here) (canonical `.koi/` layout). Sandbox: [Sandbox overview](/docs/sandbox/overview). Commands: [cheatsheet](/docs/reference/cheatsheet).
...
The full tree lives on [Start here](/docs/start-here#two-folders). Framework ADR: [0004: project home vs run](https://github.com/getkoi/koi/blob/main/docs/adr/0004-project-home-and-run-working-folder.md). Use `--working-folder` to point at the volatile folder (default `.koi/run`).
...
Read [Sessions](/docs/yokai/sessions) for the lifecycle and interruption scenarios. Use the settings below to configure that behavior.
...
occupancy. The [TUI bars](/docs/yokai/tui#context-gauge) show the latest known
...
Durable sandbox declaration at **project home**, not under `run/`. Written by `koi setup`; read by `yokai` and `koi doctor`. Flat keys (no nested `sandbox:` block in `config.yml`).

| Field | Default when omitted |
|-------|----------------------|
| `enabled` | `false` |
| `backend` | `docker` (`docker` \| `apple`) |
| `name` | `yokai-<repo-basename>` |
| `image` | required when `enabled: true` |
| `memory` | `8g` (Apple only) |
| `cpus` | `4` (Apple only) |

When `enabled: true`, bare `yokai` is a thin launcher into a Linux VM built from `.koi/Dockerfile.sandbox`. Use `yokai --host` to force the local supervisor.
...
Walkthroughs: [Launch a sandbox](/docs/sandbox/launch).

## `.koi/sensors/`

The verification bar lives in durable **project home**, not the working folder. `koi setup` creates the folder with a README pointer only; the scripts come from the yokai skill or `/junji plan`. One atomic concern per `*.sh`; exit 0 means pass. The **gate** is green when every script passes (sorted by filename, fail-fast).

Empty or missing folder means the gate fails closed. Recipes: [/recipes](/recipes). Semantics: [sensors](/docs/sensors).

Framework ADR: [0005: atomic sensors folder](https://github.com/getkoi/koi/blob/main/docs/adr/0005-atomic-sensors-folder.md).
...
## Optional project-home hooks

| Path | When |
|------|------|
| `.koi/Dockerfile.sandbox` | Scaffolded write-if-absent when `koi setup` enables sandbox |
| `.koi/sandbox-setup.sh` | Runs once per named sandbox for extras the image does not cover |

## Reconfigure

Re-running `koi init` loads existing `config.yml` as wizard defaults and rewrites only volatile driver config. It never overwrites a filled `CONTEXT.md`, live `BACKLOG.md`, or sensor scripts. To change the durable declaration (`sandbox.yml`, Dockerfile scaffold), re-run `koi setup`.
