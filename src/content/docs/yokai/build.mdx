...
With [token savings](/yokai/docs/configuration#token-savings), planning, coding and sealing
share a session, corrections return to it, and repeated reviews share a separate session.
See the [context-window diagrams](/yokai/docs/configuration#context-windows-with-and-without-token-savings)
...
**Budget:** `max_iters` (omit in `yokai.yml` for unlimited; `--max-iters` still a positive cap). Exhaustion without convergence means `not-converged`. `0` still means zero iterations.
...
Use [swarms](/yokai/docs/swarms) to split work among parallel workers and merge their output. Both the run setting and a `[swarm]` phase tag must enable them. Plain phases use one coder.

The swarm guide covers setup, worker limits, review, failures, and interrupted work. For fresh and reused conversations, read [Sessions](/yokai/docs/sessions).
...
- [Run](/yokai/docs/run): flags, halt conditions
- [Recovery](/yokai/docs/recovery): when Build does not converge
- [junji guide › next](/junji/docs/guide): human `next` protocol
- [Sensors](/yokai/docs/sensors): authoring `.koi/sensors/*.sh`
