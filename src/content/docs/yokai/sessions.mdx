...
Run `yokai --token-savings` to reuse conversations within each phase. For a persistent setting, use [token savings configuration](/yokai/docs/configuration#token-savings).
...
Use [swarms](/yokai/docs/swarms) when you want parallel workers. Use token savings when you want to retain context between turns instead.
...
Swarm continuation also checks saved work items and worker records. See [continue a swarm](/yokai/docs/swarms#continue-an-interrupted-swarm).

## Inspect what happened

Use the [TUI sessions view](/yokai/docs/tui) or inspect `.koi/run/sessions.jsonl`. The file records turns. Several rows can share a session ID, so its row count is not a count of unique conversations.

For exact session settings and the existing context-window diagrams, see [configuration](/yokai/docs/configuration#context-windows-with-and-without-token-savings). If the run cannot advance, see [recovery](/yokai/docs/recovery).
