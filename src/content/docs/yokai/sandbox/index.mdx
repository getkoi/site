...
The repository owns the OCI image (coding agent + gate tools baked in via `.koi/Dockerfile.sandbox`). koi does not ship a framework image catalog. Copy-ready `sandbox.yml` + `Dockerfile.sandbox` pairs live at [/yokai/recipes](/yokai/recipes#sandboxes).

Sandbox files live in **project home** and survive `/junji consolidate`. Driver prefs stay in `.koi/run/config.yml`. There is no `sandbox:` block there. Layout: [Start here](/koi/docs).
...
| [sandbox.yml and image](/yokai/docs/sandbox/sandbox-yml) | Declaration fields, Dockerfile USER rules, agent bake |
