...
Use `yokai` to allow swarms and choose a worker cap, or edit the `swarm` settings in [configuration](/yokai/docs/configuration). Swarms are off when omitted. The default worker cap is 8.

Both conditions must hold: `swarm.enabled` is true and the reviewed phase assessment selects `strategy: swarm` in `.koi/run/yokai.yml`. Use `/yokai assess` or the Phase assessment screen in `yokai` to propose and approve the mapping. Plain is the default proposal; swarm needs independently mergeable work with clear ownership. Keep execution choices out of plan Markdown and ordinary headings.
