...
Use swarms to run independent work items in parallel within one phase. Yokai splits the work, runs workers, and merges their output. Parallel work can shorten a run, but splitting and merging add token use. Start with a [plain run](/docs/start-here/quickstart) before enabling them.

## Enable swarms

Use `koi init` to allow swarms and choose a worker cap, or edit the `swarm` settings in [configuration](/docs/reference/configuration). Swarms are off when omitted. The default worker cap is 8.
...
The worker cap limits both item count and concurrency. An oversized or unreadable item list from a new split, or a worker error, returns a non-converged result. Yokai does not partially merge successful workers after a worker failure. [Recovery](/docs/yokai/recovery) governs what happens next.
...
- [Sessions](/docs/yokai/sessions): fresh conversations, reused context, and reattachment.
- [Build loop](/docs/yokai/build): sensors, review findings, and convergence.
- [TUI](/docs/yokai/tui): watch workers and inspect their turns.
