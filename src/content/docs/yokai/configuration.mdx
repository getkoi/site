...
Optional fan-out inside Build (each iteration of a phase assessed as `swarm`). **Omit the block to leave it off.** `yokai configure` asks whether
...
phases:
  P2:
    ...
    # reviewed_sha256 is added by Yokai after interactive review.
```

This example is a proposal until reviewed. It inherits durable guides, timeouts and sandbox
...
If sensors already exist, Yokai offers direct review without an agent turn. Decline the offer to
propose changes, inspect the existing scripts against the displayed bar, and confirm their command
sources, coverage and prerequisites before smoke execution. Final approval records fingerprints
without rewriting the scripts. Missing review metadata does not mean sensors are missing. Agent
...
copies of every proposed sensor, using the normal executor from the repository root, on the host or
configured sandbox. Scripts must rely on that cwd rather than derive the root from their file path.
No sensors or review metadata are saved until the results receive final approval. Declining either
review leaves existing sensors intact. Temporary script copies are removed after the review.
...
The durable `gate_review` mapping in `.koi/yokai.yml` contains `version: 1`, `bar_sha256` and a
`scripts` mapping of filename to SHA-256. It stores no duplicate script bodies. Changing the
verification section or adding, removing or editing a sensor invalidates review; unrelated CONTEXT
sections do not. The driver rechecks this binding during execution. Do not hand-author fingerprints.

## Phase assessment

`/yokai assess` and interactive `yokai assess` propose a model-capability tier and Build strategy for
...
Review the proposed `phases.<ID>.tier`, `strategy` and short `reason` in `.koi/run/yokai.yml`.
Existing choices are shown and preserved unless scope justifies a change. Yokai adds a
`reviewed_sha256` fingerprint after approval. Chat-assisted edits may propose values but must not
forge that fingerprint. These runtime choices never belong in PLAN or phase-plan Markdown.
Junji alone does not require the mapping. Reviewed YAML overrides optional legacy heading tags.

If every unfinished phase already has a tier, strategy and nonempty reason, decline the offer for
agent-proposed revisions to review that mapping directly against current scope. Final approval
records fingerprints without changing the choices. A helper-written mapping without fingerprints
is assessed but awaiting recorded review, not stale. A fingerprint mismatch is reported separately
as stale scope or choices. Neither case requires regeneration; incomplete mappings still need work.

Interactive setup proposes missing or stale assessments. New phases and changed phase scope require
review; removed or renumbered IDs require explicit mapping cleanup. Fingerprints bind the phase
title, gated flag, legacy annotations, backlog body, verification bar, any existing `Goal & scope`
plan section, and the reviewed choices. Status-only progress, plan outcomes and carry-forward notes
outside the phase block do not invalidate them. Creating or changing a plan's scope requires review
on a later launch if that phase is still unfinished.

Headless startup and sandbox re-entry stop on missing/stale assessments with instructions to run
interactive `yokai assess`; they never choose defaults or stamp metadata. The supervisor checks
scope again before starting the next phase. Assessment does not grant `[gated]` authorization;
`--allow-gated` remains an explicit launch choice subject to standing directives.
...
`--prefer` cannot approve changed checks. Reconcile through `yokai sensors`. Ordinary headless
...
not manufacture phase assessments; run interactive `yokai assess` for an existing effort.
