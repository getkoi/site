...
Prepare the method files with Junji, then run `yokai`. Interactive startup handles missing setup, sensor review and phase assessment. Use the focused commands when needed.
...
`sensors` reviews commands, smoke-runs temporary checks, then saves only after approval.
`assess` reviews phase tiers and strategies in effort YAML. Both require an interactive terminal.
`guidance` prints the bundled helper policy without setup. Headless execution requires a reviewed
gate and current phase assessments.
