...
- **junji skill** installed: `npx skills add getkoi/koi --skill junji`
- No CLI, runtime configuration, or sensor scripts are required for Junji.
...
- Runnable verification commands documented in CONTEXT; optional existing sensors can supply them
...
**Prerequisites:** The Junji skill. `begin` creates missing scaffolding and preserves customized files.
...
4. Leaves optional Yokai configuration and existing sensor scripts untouched
...
6. Document the project’s runnable verification commands and expected exit codes. Existing sensors may be referenced. Yokai can later propose atomic scripts for review.

**Verification:** CONTEXT contains runnable commands for the verification bar. `Next action: /junji refine`.

**Failure:** Judgment-call "bar" with no exit-code commands. Rewrite it as runnable checks: the project's real stack gates (fmt/lint/typecheck/tests), not a bespoke growing assertion script.
...
4. Use ordinary status headings such as `### [todo] P0 - Title`, with dependencies and human gates. Runtime strategy and tier choices are optional Yokai settings.
5. Write the phase list to `BACKLOG.md` only. No code or phase plans yet.

**Verification:** Ordered, independently verifiable phases; `Next action: /junji next`.
...
**Prerequisites:** Phase list in `BACKLOG.md`; runnable verification bar in CONTEXT.

A backlog heading looks like `### [todo] P0 - Title`.
...
Interactive `next` preserves user text and any legacy runtime annotations. It does not switch the agent or model.
...
5 verify     run every command in CONTEXT’s verification bar — all must pass
...
Invoke: `/junji next` or "do the next task". The **yokai driver** splits this into Plan → Build → Seal. See [Build](/yokai/docs/build).
...
- `[gated]` → human decision; an unattended launch requires explicit `yokai --allow-gated`
...
2. Reason, then **grill** until phase boundaries, dependencies, verification and human gates are settled.
3. Write ordinary `[todo]` headings continuing `P{n+1}` and a full `phases/phase-NN-<slug>.md` for each. No runtime tags or product code are required.
4. Commit the working-folder artifacts, honoring standing overrides. `Next action: /junji next`.

**Verification:** New `[todo]` phases with matching phase files; pointer is `/junji next`. Then `next` or `yokai` (yokai skips Plan because the files exist).
...
3. `git rm -r .koi/run/` in one commit. Durable `.koi/yokai.yml`, reviewed sensors, gate receipt and sandbox assets stay. Effort overrides, ledgers and sessions under `run/` are removed.
...
Optionally install **Yokai** and run `yokai` from the prepared repository. Interactive startup configures runtime preferences, proposes sensors for review, checks readiness and offers to start. Execution does not require an installed Junji skill. Once prepared, Yokai loops through authorized phases:
...
| Question | Answer |
|----------|--------|
| Where do I start? | [Quickstart](/koi/docs/quickstart): install Junji, then `/junji begin` |
...
- [Skills reference](/junji/docs/skills): junji vs yokai skill, sensor wiring
- [Sensors](/yokai/docs/sensors): gate semantics
- [yokai run](/yokai/docs/run): driver flags and contract details
